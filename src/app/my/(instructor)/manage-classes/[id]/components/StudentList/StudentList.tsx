import StudentCard from '@/app/my/(instructor)/manage-classes/[id]/components/StudentCard/StudentCard';
import {
  pannelTitleWrapper,
  studentListWrapper,
  textStyle,
} from '@/app/my/(instructor)/manage-classes/[id]/components/StudentList/StudentList.css';
import type { TabStatus } from '@/app/my/(instructor)/manage-classes/[id]/types/api';
import type { Student } from '@/app/my/(instructor)/manage-classes/[id]/types/api';
import { STATUS_KOREAN_MAP } from '@/app/my/(student)/classes/constants/statusMap';
import type { ReservationStatus } from '@/app/my/(student)/classes/types/reservationStatus';
import Divider from '@/common/components/Divider/Divider';
import Text from '@/common/components/Text/Text';

interface StudentListProps {
  reservationStatus: ReservationStatus;
  studentList: Student[];
  lessonId: number;
  selectedTab: TabStatus;
}

const StudentList = ({ reservationStatus, studentList, lessonId, selectedTab }: StudentListProps) => {
  return (
    <>
      {studentList.length > 0 && (
        <div>
          <div className={pannelTitleWrapper}>
            <Text tag="b2_sb" className={textStyle}>
              {STATUS_KOREAN_MAP[reservationStatus]}
            </Text>
            <Divider color="black" />
          </div>

          <ul className={studentListWrapper}>
            {studentList.map((student, index) => (
              <StudentCard index={index} studentData={student} lessonId={lessonId} selectedTab={selectedTab} />
            ))}
          </ul>
        </div>
      )}
    </>
  );
};

export default StudentList;
