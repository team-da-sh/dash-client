import type { UseFormRegister } from 'react-hook-form';
import * as styles from '@/app/mypage/(student)/reservations/[id]/cancel-confirm/components/RefundAccountSection/refundAccountSection.css';
import Head from '@/common/components/Head/Head';
import Input from '@/common/components/Input/Input';
import Text from '@/common/components/Text/Text';
import SvgIcArrowDownGray1032 from '@/shared/assets/svg/IcArrowDownGray1032';
import { MAX_ACCOUNT_NUMBER_LENGTH } from '@/shared/constants/account';
import { ONLY_NUMBER } from '@/shared/constants/regex';

interface Bank {
  bankId: number;
  bankName: string;
  bankImageUrl: string;
}

interface AccountFormData {
  accountNumber: string;
  bank: { bankId: number; bankName: string; bankImageUrl: string };
}

interface RefundAccountSectionProps {
  isVisible: boolean;
  selectedBank: Bank;
  onBankSelectClick: () => void;
  register: UseFormRegister<AccountFormData>;
}

const RefundAccountSection = ({ isVisible, selectedBank, onBankSelectClick, register }: RefundAccountSectionProps) => {
  const registerAccount = register('accountNumber', {
    onChange: (e) => {
      if (!ONLY_NUMBER.test(e.target.value)) {
        e.preventDefault();
        return;
      }
    },
  });

  if (!isVisible) return null;

  return (
    <div className={styles.containerStyle}>
      <div className={styles.titleContainerStyle}>
        <Head tag="h6_sb" color="black">
          환불받을 계좌
        </Head>

        <div className={styles.inputContainerStyle}>
          <button type="button" className={styles.bankSelectContainerStyle} onClick={onBankSelectClick}>
            {selectedBank.bankImageUrl && selectedBank.bankName ? (
              <div className={styles.bankInfoContainerStyle}>
                <img src={selectedBank.bankImageUrl} alt="은행 로고" className={styles.bankSelectImageStyle} />
                <Text tag="b2_sb_long">{selectedBank.bankName}</Text>
              </div>
            ) : (
              <Text tag="b2_sb_long" color="gray5">
                은행 선택
              </Text>
            )}
            <SvgIcArrowDownGray1032 width={'3.2rem'} />
          </button>
          <Input
            backgroundColor="white"
            placeholder="계좌번호 입력"
            inputMode="numeric"
            {...registerAccount}
            maxLength={MAX_ACCOUNT_NUMBER_LENGTH}
          />
        </div>
      </div>
    </div>
  );
};

export default RefundAccountSection;
