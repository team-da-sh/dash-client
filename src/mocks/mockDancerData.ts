export const DANCER_DATA = {
  imageUrl:
    'https://hankki-prod-bucket.s3.ap-northeast-2.amazonaws.com/dummy/%E1%84%80%E1%85%A9%E1%84%85%E1%85%A7%E1%84%83%E1%85%A2+%E1%84%86%E1%85%B5%E1%86%AF%E1%84%91%E1%85%B3%E1%86%AF%E1%84%85%E1%85%A2%E1%86%AB%E1%84%87%E1%85%B5.jpeg',
  nickname: '박재연',
  instagram: 'https://www.instagram.com/wannna_go_home/',
  youtube: 'https://www.youtube.com/@youthisuz',
  genre: ['힙합', '코레오', 'K-POP'],
  detail:
    '안녕하세요, 안무가 바다입니다. 춤을 통해 제 에너지를 표현하고, 많은 사람들에게 감동을 전하는 것을 목표로 활동하고 있어요. 힙합과 팝핑을 주 장르로 삼아 다양한 무대에서 퍼포먼스를 선보이며, 국내외 아티스트와의 협업을 통해 안무가로서도 성장해 왔습니다. 춤을 사랑하는 마음으로 꾸준히 도전하며, 제 춤이 여러분에게 영감을 줄 수 있기를 바랍니다. 앞으로도 더 많은 무대에서 멋진 모습을 보여드릴게요! ',
  classes: [
    {
      id: 61,
      imageUrl:
        'https://hankki-prod-bucket.s3.ap-northeast-2.amazonaws.com/dummy/%E1%84%80%E1%85%A9%E1%84%85%E1%85%A7%E1%84%83%E1%85%A2+%E1%84%86%E1%85%B5%E1%86%AF%E1%84%91%E1%85%B3%E1%86%AF%E1%84%85%E1%85%A2%E1%86%AB%E1%84%87%E1%85%B5.jpeg',
      status: 'FINISHED',
      name: '한 줄 짜리 제목입니다.',
      lessonStartDateTime: '2025-02-17T20:00:00',
      lessonGenre: 'K-POP',
      lessonLevel: '고급',
    },
    {
      id: 62,
      imageUrl:
        'https://hankki-prod-bucket.s3.ap-northeast-2.amazonaws.com/dummy/%E1%84%80%E1%85%A9%E1%84%85%E1%85%A7%E1%84%83%E1%85%A2+%E1%84%86%E1%85%B5%E1%86%AF%E1%84%91%E1%85%B3%E1%86%AF%E1%84%85%E1%85%A2%E1%86%AB%E1%84%87%E1%85%B5.jpeg',
      status: 'FINISHED',
      name: '한 줄은 너무 짧다고 생각합니다 두 줄짜리 대령이오.',
      lessonStartDateTime: '2025-01-18T06:00:00',
      lessonGenre: '힙합',
      lessonLevel: '초급',
    },
    {
      id: 63,
      imageUrl:
        'https://hankki-prod-bucket.s3.ap-northeast-2.amazonaws.com/dummy/%E1%84%80%E1%85%A9%E1%84%85%E1%85%A7%E1%84%83%E1%85%A2+%E1%84%86%E1%85%B5%E1%86%AF%E1%84%91%E1%85%B3%E1%86%AF%E1%84%85%E1%85%A2%E1%86%AB%E1%84%87%E1%85%B5.jpeg',
      status: 'FINISHED',
      name: '강의 제목',
      lessonStartDateTime: '2025-01-20T10:00:00',
      lessonGenre: '피메일 힙합',
      lessonLevel: '입문',
    },
    {
      id: 64,
      imageUrl:
        'https://hankki-prod-bucket.s3.ap-northeast-2.amazonaws.com/dummy/%E1%84%80%E1%85%A9%E1%84%85%E1%85%A7%E1%84%83%E1%85%A2+%E1%84%86%E1%85%B5%E1%86%AF%E1%84%91%E1%85%B3%E1%86%AF%E1%84%85%E1%85%A2%E1%86%AB%E1%84%87%E1%85%B5.jpeg',
      status: 'FINISHED',
      name: '끝내주는 웨이브와 에스파 슈퍼노바 일주일에 걸쳐 야무지게 배우기',
      lessonStartDateTime: '2025-08-17T20:00:00',
      lessonGenre: 'K-POP',
      lessonLevel: '고급',
    },
  ],
  education: ['숭실대학교 무용학석사', '솝트대학교 서버학과'],
  experience: ['33기 서버', '34기 서버', '35기 서버'],
  videoUrl: ['https://www.youtube.com/watch?v=DiyBJqGTmgE', 'https://www.youtube.com/watch?v=wqw9A88tliE','https://www.youtube.com/watch?v=Raz30d61HPM'],
};